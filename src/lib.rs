...
pub(crate) const MAX_OP_STRENGTH_SCORE: OpScore = OpScore(100.0);
...
pub(crate) struct ZxScore(pub f32);
...
pub(crate) struct OpScore(pub f32);
...
#[allow(dead_code)] // used for certain tests, diagnostics
impl ZxScore {
...
impl GenericPoint {
    ...
    fn new_from_point(p: &Point) -> GenericPoint {
        GenericPoint {
            x: p.zx.value() as f64,
            y: p.op.value() as f64,
        }
...
impl Point {
    ...
    #[allow(dead_code)] // This is called from a test function
    fn line_from_points(&self, p2: &Point) -> Option<Box<dyn Fn(ZxScore) -> OpScore>> {
...
mod tests {
    ...
    // anything. It is called by various test functions
